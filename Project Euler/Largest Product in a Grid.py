a = [8,2,22,97,38,15, 0, 40, 00, 75, 4, 5, 7, 78, 52, 12, 50, 77, 91, 8]
...
def horizontalMAX(array):  # ASSUming array is a 20x20.
    MAX = 0
    for j in range(0,20):
        for i in range(0,16):
            product = array[j][i]
...
def maxproduct(row): # Max product of length 4 in the row vector.
    if len(row) > 4:
        MAX = 0
        for k in range(0, len(row)-4):
            product = row[k]
            for j in range(1,4):
                product = product * row[k+j]
...
def diagonaldownMAX(array):
    MAX = 0
    for k in range(0,17):
        row = []
        i = 0
        for j in range(0, 20-k):
            row.append(array[k+j][i])
            i += 1

        product = maxproduct(row)
        if product > MAX:
            MAX = product
    for k in range(0,17):
        row = []
...
print(max(([diagonaldownMAX(array),diagonalupMAX(array),downMAX(array),horizontalMAX(array)])))

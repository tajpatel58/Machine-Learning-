...
% a_3 now encodes the output layer activations. 

for i = 1:m
    y_vec = zeros(num_labels,1);
    y_vec(y(i)) = 1;
    J = J + (y_vec' * log(a_3(i,:))' + (1-y_vec)' * log(1-a_3(i,:))');
end 

J = -1/m * J; 

% Now we need to add the regularized term. 
...
% Initialise the derivative matrices. 
...
for i = 1:m 
    ...
    d_3 = a_3(i,:)' - y_vec;
    d_2 = Theta2' * d_3 .* sigmoidGradient([1;z_2(i,:)']);
    
    
    Delta_1 = Delta_1 + d_2(2:end) * X(i,:);
    Delta_2 = Delta_2 + d_3 * a_2(i,:);
end 
